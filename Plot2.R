...
filteredHouseData <- household[household$Date %in% c("1/2/2007","2/2/2007") ,]
#Combing the Date and Time using paste function and the using the Strptime to convert it  D M Y H M S format

datetime<-strptime(paste(filteredHouseData$Date,filteredHouseData$Time,sep= " "),"%d/%m/%Y %H:%M:%S")
#using the png as gravice device
png("plot2.png", width=480, height=480)

#using plot to generate the plot as requested
plot(datetime,as.numeric(filteredHouseData$Global_active_power),type = "l",xlab="",ylab = "Global Active Power (kilowatts)")
